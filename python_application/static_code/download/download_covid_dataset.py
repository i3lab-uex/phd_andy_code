...
def download_covid_dataset() -> int:
    ...
    if (
        Path("datasets/covid/COVID-19-CT-Seg_20cases").exists()
        and Path("datasets/covid/Lung_Mask")
        and any(Path("datasets/covid/COVID-19-CT-Seg_20cases").rglob("*.nii*"))
        and any(Path("datasets/covid/Lung_Mask").rglob("*.nii*"))
    ):
        return 1
    ...
    for url in [
        "https://zenodo.org/record/3757476/files/COVID-19-CT-Seg_20cases.zip",
