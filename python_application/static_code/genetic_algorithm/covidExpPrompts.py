import sys, argparse, time, random, glob, os, gc, torch, json
import numpy as np
...
def parse_json(jsonpath,name,slice):
	...
	box_coor= [data["prompt"]["bounding_box"]["upper_left_corner"]["row"],data["prompt"]["bounding_box"]["upper_left_corner"]["column"],data["prompt"]["bounding_box"]["bottom_right_corner"]["row"],data["prompt"]["bounding_box"]["bottom_right_corner"]["column"]]
...
for element in files:
	...
	for slice_number in range(0,image.shape[2]):
		...
		if labels.size > 1:
			...
			output.write(bname+"_"+str(slice_number)+"\t"+str(objective)+"\t"+str(jaccard)+"\t"+str(dice)+"\t"+str(scoreG)+"\t"+str(part_time)+"\t"+str(s)+"\n")
