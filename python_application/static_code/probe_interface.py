...
class ProbeInterface:
    ...
    def display_probe(self):
        ...
        for ds in self.probe.dataset:
            ...
            for ss in ds.subset:
                ...
                output.append(f"    Data folder: {ss.dataFolderName}, Labels folder: {ss.labelsFolderName}")
                for sp in ss.sample:
                    output.append(f"      🖼️ Sample: {sp.filename} (Format: {sp.extension})")
        ...
        for i, ot in enumerate(self.probe.optimization_task):
            ...
            for e in ot.experiment:
                ...
                output.append(f"      Prompt Type: {type(e.initial_state.prompt).__name__}")
                if hasattr(e.initial_state.prompt, 'bounding_box') and e.initial_state.prompt.bounding_box:
                ...
                if hasattr(e.initial_state.prompt, 'point') and e.initial_state.prompt.point:
                ...
                output.append(f"      Sample: {e.sample.filename} ({e.sample.extension})")
    ...
    def update_experiment_choices(self, task_name):
        ...
        selected_task = next((task for task in self.probe.optimization_task if task.name == task_name), None)
        if selected_task:
            experiment_names = [exp.name for exp in selected_task.experiment]
            return gr.Dropdown(choices=experiment_names, value=experiment_names[0] if experiment_names else None)
        return gr.Dropdown(choices=[], value=None)
        ...
        with gr.Blocks() as interface:
            ...
            with gr.Tabs():
                with gr.TabItem("👁️ PROBE Viewer"):
                    ...
                    display_btn.click(fn=self.display_probe, inputs=[], outputs=output_textbox)
                    save_status = gr.Textbox(label="Filename saved", lines=1, interactive=False)
                    save_btn = gr.Button("Save logs")
                    save_btn.click(fn=self.save_logs_to_file, inputs=None, outputs=save_status)
                ...
                with gr.TabItem("⚙️ Run Algorithms"):
                    ...
                    with gr.Row():
                        default_experiments = [exp.name for exp in self.probe.optimization_task[0].experiment] if self.probe.optimization_task else []
                        experiment_selector = gr.Dropdown(
                            label="Select an experiment",
                            choices=default_experiments,
                            value=default_experiments[0] if default_experiments else None
                        )
